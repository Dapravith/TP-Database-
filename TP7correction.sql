...
;
#5.	Some directors directed more than one movie. For all such directors, return the titles of all movies directed by them, along with the director name. Sort by director name, then movie title. 
select title, movie.director
from movie natural join 
	(select director from movie group by director having count(mid) > 1) as temp
...
from movieavgrating
where (director,avgstars) in 
	(select director, maxstars
